...
import json
import re
from collections import deque   # <-- this one is critical
from dotenv import load_dotenv
from openai import OpenAI
from src.chroma_utils import query_collections
...
class AgenticRAG:
    ...
    def analyze_query(self, query):
        """
        Uses LLM to analyze and decompose the query into sub-queries.
        Returns:
            tuple: (is_complex, sub_queries)
        """
        try:
            prompt = f"""
            You are a query decomposition assistant.
            Analyze the following user query and decide whether it is complex 
            (i.e., contains multiple sub-questions or requires breaking down).
            ...
            - If the query is simple, set is_complex=false and sub_queries=[the original query].
            - If complex, split into the minimum meaningful sub-queries.
            """
            ...
            # Extract JSON (in case model adds explanation)
            json_match = re.search(r'\{.*\}', raw_output, re.DOTALL)
            if json_match:
                raw_output = json_match.group(0)

            parsed = json.loads(raw_output)
            return parsed.get("is_complex", False), parsed.get("sub_queries", [query])

        except Exception as e:
            print(f"LLM query analysis failed, fallback to simple mode: {e}")
            return False, [query]
